        ...
        for (int indexRun = 0; indexRun < numRuns; indexRun++) {
            ...
            for (int i = 0; i < PNRStop.getDepartureDelayList().size(); i++) {
                if (PNRStop.getDepartureDelayList().get(i) > 60)
                    numTrainDelay++;
                if (PNRStop.getDepartureDelayList().get(i) > maxDelay)
                    maxDelay = PNRStop.getDepartureDelayList().get(i);
...
System.out.println("Trams at CS"+ CS);
        ...
        for (int i = 0; i < CS; i++) {
            ...
            Tram tram = new Tram(i + 1, CSStop, CSStop, t);
        ...
        for (int i = 0; i < PNR; i++) {
            LocalTime t = LocalTime.of(simulationStartTime.getHour(), simulationStartTime.getMinute(), simulationStartTime.getSecond());
            t = t.plusMinutes((i * 60 / frequency) - 1);
            Tram tram = new Tram(i +1, PNRStop, PNRStop, t);
            tram.setPlannedArrivalTime(t);
            tram.setDirection(0);
            TramEvent arriving = new TramEvent(5, t, tram);
    ...
    public static void initStops(List<Stop> routeCSPNR, List<Stop> routePNRCS, List<Stop> stopList, EndStop CSStop, EndStop PNRStop) {
        Stop stopB7 = new Stop("Vaartsche Rijn",  17);
        Stop stopB6 = new Stop("Galgenwaard", 16);
        Stop stopB5 = new Stop("De Kromme Rijn", 15);
        Stop stopB4 = new Stop("Padualaan", 14);
        Stop stopB3 = new Stop("Heidelberglaan", 13);
        Stop stopB2 = new Stop("UMC", 12);
        Stop stopB1 = new Stop("WKZ", 11);

        Stop stopA1 = new Stop("Vaartsche Rijn", 2);
        Stop stopA2 = new Stop("Galgenwaard", 3);
        Stop stopA3 = new Stop("De Kromme Rijn", 4);
        Stop stopA4 = new Stop("Padualaan", 5);
        Stop stopA5 = new Stop("Heidelberglaan", 6);
        Stop stopA6 = new Stop("UMC", 7);
        Stop stopA7 = new Stop("WKZ", 8);
